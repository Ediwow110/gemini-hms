...
export class DeviceMeshGateway {
  ...
  async handleDeviceIngest(payload: Buffer): Promise<ParsedTelemetry> {
    this.logger.log(`Received remote IoT telemetry buffer stream: Length ${payload.length} bytes`);

    // Minimum package length: 36 (tenantId) + 16 (deviceId) + 4 (float value) + 36 (authToken) = 92 bytes
    if (payload.length < 92) {
      throw new Error(`CRITICAL: Fragmented packet rejected. Expected >= 92 bytes, got ${payload.length}`);
    }
    ...
    this.logger.log(`Parsed IoT metrics: Device [${deviceId}] | Tenant [${tenantId}] | Val: ${telemetryValue}`);
    ...
    if (authToken !== expectedHash) {
      this.logger.error(`🚨 [TENANT_VIOLATION] Hardware auth signature rejected! Unauthorized access to Tenant ID ${tenantId}.`);
      throw new Error(`Security Exception: Multi-tenant tenantId namespace boundary breach detected!`);
    }
    ...
    this.logger.log(`🟢 [REDIS_CACHE] Buffered telemetry under key "${redisKey}" (Latency < 2ms)`);
   ...
   */
  private triggerAsynchronousPostgresWrite(key: string, record: ParsedTelemetry) {
    setImmediate(() => {
      // Simulate non-blocking Prisma write to database
      record.syncedToPostgres = true;
      this.logger.log(`🟢 [POSTGRES_SYNC] Dynamic background ledger write completed for target "${key}".`);
    });
  }
}
