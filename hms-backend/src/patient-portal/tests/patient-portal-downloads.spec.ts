...
describe('PatientPortalService Downloads and Requests', () => {
  ...
  beforeEach(async () => {
    prisma = {
      tenant: { findUniqueOrThrow: jest.fn().mockResolvedValue({ id: 'tenant-1', name: 'Test Tenant' }) },
  ...
  describe('Lab Results', () => {
    it('should allow patient to download own released lab result and log audit', async () => {
      const mockResult = { id: 'result-1', order: { patient: { id: 'patient-1' } } };
      prisma.labResult.findFirst.mockResolvedValue(mockResult);

      const res = await service.getLabResultForExport('tenant-1', 'patient-1', 'result-1');
    ...
    it('should block if lab result is unreleased or wrong patient', async () => {
      ...
      await expect(service.getLabResultForExport('tenant-1', 'patient-1', 'result-1'))
  ...
  describe('Invoices', () => {
    it('should allow patient to download own posted invoice and log audit', async () => {
      const mockInvoice = { id: 'inv-1', order: { patient: { id: 'patient-1' } } };
      prisma.invoice.findFirst.mockResolvedValue(mockInvoice);

      const res = await service.getInvoiceForExport('tenant-1', 'patient-1', 'inv-1');
    ...
    it('should block if invoice is wrong patient or missing', async () => {
      ...
      await expect(service.getInvoiceForExport('tenant-1', 'patient-1', 'inv-1'))
        .rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('Receipts', () => {
    it('should allow patient to download own paid receipt and log audit', async () => {
      const mockPayment = { id: 'pay-1', invoice: { order: { patient: { id: 'patient-1' } } } };
      prisma.payment.findFirst.mockResolvedValue(mockPayment);

      const res = await service.getPaymentForReceipt('tenant-1', 'patient-1', 'pay-1');
    ...
    it('should block if receipt is unpaid or wrong patient', async () => {
      ...
      await expect(service.getPaymentForReceipt('tenant-1', 'patient-1', 'pay-1'))
        .rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('Prescriptions', () => {
    it('should allow patient to download own valid prescription and log audit', async () => {
      ...
      const res = await service.getPrescriptionForExport('tenant-1', 'patient-1', 'rx-1');
    ...
    it('should block if prescription is draft/cancelled or wrong patient', async () => {
      ...
      await expect(service.getPrescriptionForExport('tenant-1', 'patient-1', 'rx-1'))
        .rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('Refill Requests', () => {
    it('should validate ownership and create trackable request, logging audit', async () => {
      ...
      await service.createRefillRequest('tenant-1', 'patient-1', 'rx-1', { reason: 'need more' });
      expect(audit.log).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should block duplicate open requests', async () => {
      ...
      await expect(service.createRefillRequest('tenant-1', 'patient-1', 'rx-1', { reason: 'need more' }))
  ...
  describe('Medical Record Requests', () => {
    it('should create medical record request and log audit', async () => {
      ...
      await service.createMedicalRecordRequest('tenant-1', 'patient-1', { requestType: 'FULL_RECORD', reason: 'moving' });
      expect(audit.log).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should block duplicate requests', async () => {
      ...
      await expect(service.createMedicalRecordRequest('tenant-1', 'patient-1', { requestType: 'FULL_RECORD' }))
        .rejects.toThrow(ConflictException);
    });
  });
});
