...
export class CognitiveRehabService {
  ...
  private seedRegistry() {
    this.deviceRegistry.set('tactile-block-01', {
      deviceId: 'tactile-block-01',
      tenantId: '234f5c00-f6a3-4d55-996a-281e1306d7ca',
    });
    this.patientStates.set('pat-777', {
      patientId: 'pat-777',
      tenantId: '234f5c00-f6a3-4d55-996a-281e1306d7ca',
      complexityTier: 3,
  ...
  async processTherapyPacket(
    tenantId: string,
    payload: RehabSessionDto,
  ): Promise<PatientRehabState> {
    this.logger.log(
      `Processing cognitive rehab packet for patient: ${payload.patientId}`,
    );
    ...
    if (!device || device.tenantId !== tenantId) {
      throw new ForbiddenException(
        'Device is not registered or assigned to a different tenant namespace.',
      );
    }
    ...
    if (!state || state.tenantId !== tenantId) {
      throw new ForbiddenException(
        'Patient state not found or isolated tenant mismatch.',
      );
    }
    ...
    if (payload.isSessionAborted) {
      this.logger.error(
        `🚨 [ATOMIC_ROLLBACK] Device disconnected mid-session mutation loop. Safely rolling back session logs to preserve data integrity.`,
      );
    ...
    const beta = 0.6;
    const tRatio =
      payload.targetLatency / Math.max(0.001, payload.actualLatency);
    ...
    this.logger.log(
      `P_rehab precision coefficient calculated: ${pRehab.toFixed(3)}`,
    );
    ...
    if (pRehab < 0.45) {
      ...
      this.logger.warn(
        `[WARNING] Degradation captured. Consecutive low scores counter: ${state.consecutiveLowScores}`,
      );
      ...
      if (state.consecutiveLowScores >= 3) {
        if (state.complexityTier > 1) {
          ...
          this.logger.error(
            `🚨 [CRITICAL_REGRESSION_ALERT] Continuous regression anomaly intercepted! Downgrading therapeutic complexity tier to ${state.complexityTier}.`,
          );
          this.auditLogs.push({
      ...
      if (pRehab > 0.85) {
        if (state.complexityTier < 5) {
          state.complexityTier += 1;
          this.logger.log(
            `🟢 [PACING_ADVANCED] Patient excelled. Incrementing complexity tier up to ${state.complexityTier}.`,
          );
        }
      }
    }
