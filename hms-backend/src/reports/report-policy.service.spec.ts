...
describe('ReportPolicyService', () => {
  ...
  it('rejects unknown report types', () => {
    expect(() => service.getPolicyForExport('UNKNOWN_TYPE')).toThrow(BadRequestException);
  });

  it('returns valid policy for known report without specific fields', () => {
    const policy = service.getPolicyForExport('CASHIER_REVERSAL_RECONCILIATION');
  ...
  it('rejects unallowlisted fields', () => {
    expect(() => service.getPolicyForExport('CASHIER_REVERSAL_RECONCILIATION', ['unknown_field'])).toThrow(BadRequestException);
  });

  it('computes risk level based on requested fields', () => {
    const policy = service.getPolicyForExport('CASHIER_REVERSAL_RECONCILIATION', ['id', 'status', 'reason']);
  ...
  it('computes risk level as PRIVILEGED for sensitive fields', () => {
    const policy = service.getPolicyForExport('AUDIT_EVENTS_SUMMARY', ['oldValues', 'newValues']);
