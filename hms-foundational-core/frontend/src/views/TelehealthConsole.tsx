...
export const TelehealthConsole: React.FC = () => {
  ...
  const [bitrate, setBitrate] = useState(2471); // Dynamic tracked ceiling
  const [isCongested, setIsCongested] = useState(false);

  // Stream bandwidth recalibration engine loop
  useEffect(() => {
    // Mathematical tracking limit matrix: B_target = max(250, 2500 * (1 - 4.5 * L^2))
    let target = 2500 * (1 - 4.5 * Math.pow(packetLoss, 2));
    target = Math.max(250, target);
    setBitrate(Math.round(target));
