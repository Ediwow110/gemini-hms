...
export const PreventiveMaintenancePage: React.FC = () => {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-black text-slate-800 tracking-tight">Preventive Maintenance</h2>
        <p className="text-xs text-slate-500 font-medium">Scheduled service visits and calibration management</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
           ...
           {[1, 2].map((i) => (
             <div key={i} className="bg-white border border-slate-200 rounded-3xl p-6 flex items-center justify-between group">
                <div className="flex items-center gap-4">
                   <div className="h-10 w-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center">
                      <History className="h-5 w-5" />
                   </div>
                   <div>
                      <h4 className="text-sm font-black text-slate-800">Job: PM-2026-042{i}</h4>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Roche cobas c 311 · Metro Central</p>
                   </div>
                </div>
