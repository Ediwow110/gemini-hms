...
export const IntegrationDashboard: React.FC = () => {
  ...
  const { data: approvals } = useIntegrationApprovals();
  const { data: audits } = useIntegrationActivityAudit();
  const { data: issues } = useIntegrationReconciliation();
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-xl font-black text-slate-800 tracking-tight">Integration Bridges Command Center</h2>
          <p className="text-xs text-slate-500 font-medium">Cross-domain monitoring, notifications, and activity oversight</p>
        </div>
