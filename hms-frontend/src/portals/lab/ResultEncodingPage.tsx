...
export const ResultEncodingPage = () => {
  ...
  const age = context ? computeAge(context.dob) : 0;
  const dobStr = context ? new Date(context.dob).toLocaleDateString('en-US', {
    year: 'numeric', month: '2-digit', day: '2-digit',
  }) : '—';
  const panelName = context?.panelName || 'Lab Panel';
  const hasNoCatalog = context && !catLoading && !catError && (!definitions || definitions.length === 0);
    ...
    <div className="space-y-4 animate-fade-in font-sans text-slate-700">
      ...
      <HmsDashboardShell footer={<HmsAuditFooter dataSource="Laboratory LIS Service" lastRefreshed={new Date()} />}>
        {isLoading ? (
          ...
          <HmsDataUnavailable sectionName="Encoding Context" expectedApi="GET /api/v1/lab/orders/:orderId/lab-draft-context" />
        ) : !context ? (
          <HmsDataUnavailable sectionName="Encoding Context" expectedApi="GET /api/v1/lab/orders/:orderId/lab-draft-context" />
        ) : (
          <>
