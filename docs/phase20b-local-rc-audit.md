...
| **Doctor** | `/doctor/*` | Doctor | **Partial** | **Partial** | SOAP (real), patient directory (WIP), prescribing (WIP) | **SOAP**: clinical.* permissions; **Prescribing**: `inventory.stock.dispense` | tenant/branch | ✅ clinical | Patient directory mock, e-prescribing mock | **MEDIUM** |
...
| **Patient** | `/patient/*` | Patient | **Partial** | **Real** | Lab results (real), prescriptions (real), billing (partial), appointments (mock), medical records (mock) | `patient.portal.*` | patient-owned | ✅ patient-portal | Appointments mock, medical records mock, messages WIP | **LOW** |
