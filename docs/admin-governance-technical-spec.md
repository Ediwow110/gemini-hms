...
- **Governed Catalog Management**: CRUD operations for `ServiceCatalog` and `InventoryItem` are backend-governed, requiring specific permissions (`catalog.service.*` or `inventory.item.*`) and producing comprehensive audit trails for every price or status change.
- **Soft-Deactivation Lifecycle**: Both service and inventory items use a `status` model (ACTIVE/INACTIVE). Inactive items are rejected by the `OrdersService` to prevent discontinued items from being sold.
